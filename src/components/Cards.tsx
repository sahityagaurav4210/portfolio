...
const Cards: React.FC<IProjects> = ({
  ...
  const cardMedia = cardImage ? `${import.meta.env.VITE_BACKEND_BASE_URL}${cardImage}` : '/project.jpg';
  ...
  return (
    <div className='transition-all w-96 min-h-[700px] rounded-md border-2 border-dashed border-blue-500 ring-1 ring-offset-1 ring-blue-400 mx-2 mb-3 relative'>
      <div className='m-1'>
        <img src={cardMedia} alt='Laptop' className='rounded-md min-h-[250px] w-full object-cover flex justify-center' />
      </div>
      ...
      <div className='p-4'>
        ...
        <div
          className='mt-3 text-sm lg:text-lg text-gray-800 text-justify min-h-[100px] font-roboto'
        ...
        <div className='my-4 text-center font-semibold border border-b-2 border-blue-300 ring-1 ring-offset-1 ring-blue-400 bg-blue-800 shadow-inner shadow-blue-300 text-white rounded-sm px-2 py-2 w-1/3'>
          <p className='text-sm'>Project Glance</p>
        </div>

        <div className='flex gap-2 flex-wrap'>
          <Tags labelName='Project Type' content={type?.toUpperCase()} />

          <Tags labelName='Project Domain' content={projectDomain?.toUpperCase()} />
        </div>
          ...
          <div className='flex flex-col'>
            ...
            {documentation_link && (
              <button
                ...
                disabled={disabled}
                onClick={() =>
                  documentation_link ? window.open(documentation_link) : alert('No docs are available for the selected project.')
                }
